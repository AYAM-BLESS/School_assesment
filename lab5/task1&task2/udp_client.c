...
void sendRequest(){
 ...
 if(connect(socket_file_descriptor, (SA *)&serveraddress,
sizeof(serveraddress)) < 0) {
 ...
 if(quit_status == 0){
 sendto(socket_file_descriptor, message, MESSAGE_BUFFER, 0, (SA*)
NULL, sizeof(serveraddress));
...
NULL, sizeof(serveraddress));
 ...
 recvfrom(socket_file_descriptor, buffer, sizeof(buffer), 0, (SA*)
NULL, NULL);
 printf("%s\n", buffer);
 }
